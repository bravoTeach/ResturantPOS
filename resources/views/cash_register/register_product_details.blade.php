...
<div class="row">
  <div class="col-md-12">
    <hr>
    <h3>@lang('lang_v1.product_sold_details_register') (By Categories)</h3>
    <table class="table table-condensed">
        <tr>
            <!-- <th>#</th> -->
            <th>Product Name</th>
        ...
        @foreach($details['product_details_by_brand'] as $detail)
            @if($category !== $detail->category_name)
                @if($category !== null)
                    <!-- Close the previous category section if exists -->
                @endif
                @php
                    $category = $detail->category_name;
                @endphp
                <tr>
                    <td colspan="5" style="border: 1px solid black; border-radius: 10px; width: 100%; text-align: center; font-size: 20px;"><strong>{{ $category }}</strong></td>
                </tr>
            @endif
            <tr>
                <!-- <td>{{ $index++ }}.</td> -->
        ...
        <!-- Final details -->
        @if($details['transaction_details']->total_tax != 0)
        <tr class="success" style="border-top: 3px solid black">
            <th>@lang('sale.order_tax'): (+)</th>
            <th></th>
            <th>
              <span class="display_currency" data-currency_symbol="true">
                {{ $details['transaction_details']->total_tax }}
        ...
        @if($details['transaction_details']->total_shipping_charges != 0)
        <tr class="success">
            <th>@lang('lang_v1.total_shipping_charges'): (+)</th>
            <th></th>
            <th>
              <span class="display_currency" data-currency_symbol="true">
                {{ $details['transaction_details']->total_shipping_charges }}
