...
<section class="content content-custom no-print">
    ...
    @if(auth()->user()->can('dashboard.data'))
        ...
        @endif 
        <!-- my bar graph start -->
        <!-- <button id="fullscreenButton" class="btn btn-primary" style="float: right">View Fullscreen</button>
        ...
        <div class="row">
<div class="col-12">
<div class="box box-primary container" style="padding-top: 10px;">
        <button id="fullscreenButton" class="btn btn-primary" style="float: right;">View Fullscreen</button>
        <div><h4>Sale Report</h4></div>
        <div style="display: flex;justify-content: space-evenly">
            <div class="col-3 input-container">
                <label for="startDate">Start Date:</label>
                <input class="form-control" type="date" id="startDate">
...
// Set current date as the default value for both input fields
document.getElementById('startDate').value = currentDate;
document.getElementById('endDate').value = currentDate;
    
                const salesChart = initializeChart();

                // Define color mappings for each payment method
        // Define color mappings for each payment method
const colorMap = {
    cash: { backgroundColor: 'rgba(75, 192, 192, 0.6)', borderColor: 'rgba(75, 192, 192, 1)' },
    card: { backgroundColor: 'rgba(153, 102, 255, 0.6)', borderColor: 'rgba(153, 102, 255, 1)' },
    credit: { backgroundColor: 'rgba(255, 159, 64, 0.6)', borderColor: 'rgba(255, 159, 64, 1)' },
    cheque: { backgroundColor: 'rgba(255, 99, 132, 0.6)', borderColor: 'rgba(255, 99, 132, 1)' },
    bank_transfer: { backgroundColor: 'rgba(54, 162, 235, 0.6)', borderColor: 'rgba(54, 162, 235, 1)' },
    'Party Group': { backgroundColor: 'rgba(255, 205, 86, 0.6)', borderColor: 'rgba(255, 205, 86, 1)' }
};
                ...
                const fetchAndRenderData = async () => {
                    ...
                    try {
                        const response = await fetch(`/register-sales-by-user-and-payment?start_date=${startDate}&end_date=${endDate}`);
                        if (!response.ok) {
                            throw new Error('Network response was not ok');
                        }
                        const salesData = await response.json();
                        console.log(salesData);
                        updateChart(salesChart, salesData.sales_data);
                    } catch (error) {
                ...
                // Update the chart with new sales data
                function updateChart(chart, salesData) {
        ...
        let paymentMethodLabel = method;
        if (method.toLowerCase() === 'custom_pay_1') {
            paymentMethodLabel = 'Party Group';
        }
        const { backgroundColor, borderColor } = colorMap[paymentMethodLabel.toLowerCase()] || { backgroundColor: 'rgba(255, 206, 86, 0.6)', borderColor: 'rgba(255, 206, 86, 1)' };
        ...
        return {
            ...
            borderWidth: 1
                ...
                // Fetch and render data when date inputs change
