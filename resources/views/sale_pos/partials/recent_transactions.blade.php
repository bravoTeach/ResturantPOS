		...
		@foreach ($transactions as $transaction)
			<tr class="cursor-pointer" 
				...
				<td>
	    			...
	    			<a href="{{action([\App\Http\Controllers\SellPosController::class, 'printInvoice'], [$transaction->id])}}" class="print-invoice-link" onclick="window.print(); return false;">
	    				<i class="fa fa-print text-muted" aria-hidden="true" title="{{__('lang_v1.click_to_print')}}"></i>
	    			</a>

					<!-- <a href="{{ url('printer/print-kitchen/' . $transaction->id) }}" class="print-invoice-link">
						<i class="fas fa-utensils text-muted" aria-hidden="true" title="{{ __('lang_v1.click_to_print') }}"></i>
					</a> -->
				</td>
			</tr>
		@endforeach
