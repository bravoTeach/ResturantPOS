$(document).ready(function() {
    ...
    $(document).on('click', '.btn-modal', function(e) {
        ...
        if(document.getElementById('zone')){
            var zoneCheckbox = document.getElementById('zone').checked ? 1 : 0;
            myurl = $(this).data('href');
            if (myurl.includes('?')) {
                myurl = $(this).data('href') + '&chana=' + zoneCheckbox
            } else {
                myurl = $(this).data('href') + '?chana=' + zoneCheckbox
            }
        }else{
    ...
    $(document).on('click', '.delete-sale', function (e) {
        ...
        swal({
            title: LANG.sure,
            text: "Enter password:", // Prompt for password
            content: {
                element: "input",
                attributes: {
                    type: "password",
            ...
            if (willDelete) {
                var password = $('.swal-content__input').val(); // Retrieve entered password
                var dt = $('#myOrderDate').html();
                console.log(dt);
                // Parse the given date and time string into a Date object
                var givenDateTime = new Date(dt);
                ...
                var isOneHourDifference = Math.abs(differenceInHours) <= 1;
                console.log("differ is ",isOneHourDifference);
                // return;

                // Check if the difference is exactly 1 hour
                if ((password === 'Less@9092' && isOneHourDifference)|| password === 'Delete@9092') {
                    var href = $(this).attr('href');
                    var is_suspended = $(this).hasClass('is_suspended');
                    // Show success pop-up with countdown timer
                    var seconds = 5;
                    var countdown = setInterval(function () {
                        ...
                        swal({
                            title: "Deleting....!!",
                            text: "This pop-up will close in " + seconds + " seconds.",
                            icon: "success",
                            buttons: false,
                        });
                        if (seconds === 0) {
