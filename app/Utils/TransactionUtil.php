...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        if($input['is_suspend'] == 0 && isset($input['zone'])){

            if($input['status'] == 'final' && ! $is_credit_sale){

                try{
                    $response =  Http::post('http://localhost:8282/SalesInvoiceService', [
                        "posId"=> 1355,
                        "name"=> "ALA RAHI (FORMER NAME ALA RASI)",
                        "ntn"=> "8899292",
                        "invoiceDateTime"=> $input['transaction_date'],
                        "invoiceType"=>1,
                        "invoiceID"=> $invoice_no,
                        "rateValue"=> $input['tax_calculation_amount'],
                        "saleValue"=> $invoice_total['total_before_tax'],
                        "taxAmount"=> ($invoice_total['total_before_tax']/100)*$input['tax_calculation_amount'],
                        "consumerName"=> "N/A",
                        "consumerNTN"=> "N/A",
                        "address"=> "N/A",
                        "tariffCode"=> "N/A",
                    ...
                    if($responseData['resCode'] == "00"){
                        ...
                        $srb = $responseData['srbInvoceId'];
                        $msg = "Sale Added Successfully and SRB Added";
                    }else{
                        $srb = null;
                        $msg = "Sale Added Successfully";
                    }
                    $input['custom_field_1'] = $srb;
                } catch (\Exception $e) {
                    $msg = $e->getMessage();
                }
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        $transaction = Transaction::where('id', $transaction_id)
                    ...
                    $printer = PrinterController::printKitchen($transaction_id,"ORDER CANCELLED",[]);
     ...
     * @return object
     */
    public function getListSells($business_id, $sale_type = 'sell', $chana, $request)
    {
        ...
        if (!$chana) {
            ...
            $sells->whereNotNull('transactions.custom_field_1');
            $sells->orWhere('transactions.payment_status', 'due');
        }
