...
class CashRegisterUtil extends Util
{
    ...
    public function getRegisterRangeDetails($register_ids)
    {
        ...
        $query->whereIn('cash_registers.id', $register_ids);

        $lastId = end($register_ids);

        $lastRegister = CashRegister::where('id', $lastId)->first();
        ...
        $register_details['closed_at'] = $lastRegister['closed_at'];
     ...
     * @return array
     */
    public function getRegisterTransactionDetails($user_id, $open_time, $close_time, $is_types_of_service_enabled = false, $chana)
    {
        $product_details_by_brand = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transactions.updated_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
                ->where('transactions.is_direct_sale', 0)
                ->when($chana, function ($query) {
                    $query->whereNotNull('transactions.custom_field_1');
                })
                ...
                ->orderByRaw('CASE WHEN category_name IS NULL THEN 2 ELSE 1 END, category_name, product_name')
                ->get();
        
        $party_accounts = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transactions.updated_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
                ->where('transactions.is_direct_sale', 0)
    ...
    public function getCustomRangeDetails($open_time, $close_time, $is_types_of_service_enabled = false, $chana)
    {
        $product_details_by_brand = Transaction::whereBetween('transactions.updated_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
                ->where('transactions.is_direct_sale', 0)
                ->when($chana, function ($query) {
        ...
        $party_accounts = Transaction::whereBetween('transactions.updated_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
                ->where('transactions.is_direct_sale', 0)
                ->when($chana, function ($query) {
        ...
        $product_details = Transaction::whereBetween('transactions.updated_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
                ->where('transactions.is_direct_sale', 0)
                ->when($chana, function ($query) {
        ...
        $types_of_service_details = null;
        if ($is_types_of_service_enabled) {
            $types_of_service_details = Transaction::whereBetween('transactions.updated_at', [$open_time, $close_time])
                ->where('transactions.is_direct_sale', 0)
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
                ->when($chana, function ($query) {
        ...
        $transaction_details = Transaction::whereBetween('transactions.updated_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.is_direct_sale', 0)
                ->where('transactions.status', 'final')
                ->when($chana, function ($query) {
